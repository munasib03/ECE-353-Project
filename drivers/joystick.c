...
joystick_position_t joystick_get_pos(void)
{
  ...
  uint16_t y_dir = joystick_read_y(); 
   if (x_dir > JOYSTICK_THRESH_X_LEFT_2P475V && y_dir > JOYSTICK_THRESH_Y_UP_2P475V){
       return JOYSTICK_POS_UPPER_LEFT; 
    }
     else if (x_dir > JOYSTICK_THRESH_X_LEFT_2P475V && y_dir < JOYSTICK_THRESH_Y_DOWN_0P825V){
       return JOYSTICK_POS_LOWER_LEFT; 
    }
     else if (x_dir < JOYSTICK_THRESH_X_RIGHT_0P825V && y_dir > JOYSTICK_THRESH_Y_UP_2P475V){
       return JOYSTICK_POS_UPPER_RIGHT; 
    }
     else if (x_dir < JOYSTICK_THRESH_X_RIGHT_0P825V && y_dir < JOYSTICK_THRESH_Y_DOWN_0P825V){
       return JOYSTICK_POS_LOWER_RIGHT; 
    }
    else if(x_dir > JOYSTICK_THRESH_X_LEFT_2P475V){
       return JOYSTICK_POS_LEFT;
    }
     else if(x_dir < JOYSTICK_THRESH_X_RIGHT_0P825V){
       return JOYSTICK_POS_RIGHT;
    }
     else if(y_dir > JOYSTICK_THRESH_Y_UP_2P475V){
