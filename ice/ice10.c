...
Circular_Buffer_Status test_circular_buffer(Circular_Buffer *buffer, char *values, uint16_t buf_size, uint8_t test_number)
{
     ...
     */
     for(i = 0; i < buf_size; i++){
        if(circular_buffer_add(buffer, values[i]) != true){
     ...
     */
     if(circular_buffer_full(buffer) != true){
     ...
     */
    if(circular_buffer_add(buffer, 100) != false){
     ...
     */
  //  char *return_char_check;
    for (i = 0; i < buf_size; i++)
    {
    //  return_char_check[i] =  circular_buffer_remove(buffer);
      if( circular_buffer_remove(buffer) != values[i]){
        return CB_ERROR_VALUE_MISMATCH;
      }
    }  
    ...
    if(circular_buffer_empty(buffer) != true){
    ...
    if(circular_buffer_remove(buffer) != false){
